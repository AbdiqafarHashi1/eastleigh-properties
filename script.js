...
const state = { intent: 'Buy', search: '', type: 'All Types', min: '', max: '', beds: 0, features: [], sort: 'newest', visibleCount: 0, saved: new Set(), recent: [], activePropertyId: null, formOpen: false, formSuccess: false, categoryFocus: null, user: null, listingDraft: {}, draftImages: [], myListingsOpen: false, accountPanelOpen: false };
...
const getSpecValue = (property, type) => {
  if (type === 'beds') return property.category === 'Land / Plots' ? 'N/A' : `${property.beds || 0}`;
  if (type === 'baths') return property.category === 'Land / Plots' ? 'N/A' : `${property.baths || 0}`;
  if (type === 'size') return property.size || 'N/A';
  return property.category || 'N/A';
};
const polishDescription = (property) => {
  const description = (property.description || '').trim();
  if (!description) return 'A well-positioned listing presented by Eastleigh Properties with practical details available on request.';
  const clean = description.replace(/\s+/g, ' ').replace(/\s*\.\s*/g, '. ').trim();
...
function getStoredSession(){
  try{
    ...
    return { name, contact, authVersion: AUTH_STORAGE_VERSION };
  } catch { return null; }
}

function closeAccountPanel(){
  state.accountPanelOpen = false;
  document.getElementById('accountPanelModal')?.remove();
  if(!state.activePropertyId && !document.getElementById('authModal') && !document.getElementById('listingWizardModal') && !document.getElementById('myListingsModal') && !document.getElementById('listingSuccessModal')) document.body.classList.remove('modal-open');
}

function openAccountPanel(){
  if(!accountPanelBtn || !state.user) return;
  closeListingFlows();
  state.accountPanelOpen = true;
  setDrawer(false);
  const initial = (state.user?.name?.trim()?.[0] || 'A').toUpperCase();
  const modal = document.createElement('div');
  modal.id = 'accountPanelModal';
  modal.className = 'property-modal-backdrop';
  modal.innerHTML = `<section class="property-modal account-panel-modal"><header class="modal-header modal-header-clean"><button class="modal-close" aria-label="Close">×</button></header><div class="modal-body"><div class="account-panel-head"><span class="account-avatar">${initial}</span><div><h3>${state.user.name}</h3><p>${state.user.contact}</p></div></div><div class="account-panel-actions"><button type="button" class="account-menu-item" id="listAnotherBtn">List Another Property</button><button type="button" class="account-menu-item" id="myListingsBtn">My Listings</button><button type="button" class="account-menu-item danger" id="logoutBtn">Logout</button></div></div></section>`;
...
function openListingWizard(stepOverride){
  ...
  modal.innerHTML=`<section class="property-modal listing-wizard-modal"><header class="modal-header modal-header-clean"><button class="modal-close" aria-label="Close">×</button></header><div class="modal-body wizard-modal-body"><div class="stepper">${stepNames.map((n,i)=>`<span class="${i+1===step?'active':''}">${n}</span>`).join('')}</div><form id="listingWizardForm" class="wizard-form listing-wizard-form" data-step="${step}">${renderWizardStep(step,draft,imagePreviews)}<div class="wizard-actions"><button type="button" class="btn" data-wizard-back ${step===1?'disabled':''}>Back</button><button type="button" class="btn btn-muted" data-wizard-save>Save Draft</button><button type="button" class="btn btn-gold" data-wizard-next>${step===6?'Submit Listing':'Next'}</button></div></form></div></section>`;
  document.body.appendChild(modal); document.body.classList.add('modal-open');
}
...
function closeListingFlows(){['authModal','listingWizardModal','myListingsModal','listingSuccessModal'].forEach(id=>document.getElementById(id)?.remove()); document.body.classList.remove('modal-open'); closeAccountPanel();}
...
function renderModal() {
  ...
  modal.innerHTML = `<section class="property-modal" role="dialog" aria-modal="true" aria-labelledby="propertyModalTitle"><header class="modal-header"><button type="button" class="modal-close" aria-label="Close property details">×</button></header><div class="modal-body" id="modalBodyScroll"><div class="modal-layout"><div class="modal-media"><img src="${property.image}" alt="${property.title}"/><div class="thumbs">${thumbs.map((image, i) => `<img src="${image}" alt="Property view ${i + 1}"/>`).join('')}</div></div><div class="modal-content modal-content-premium"><span class="modal-intent">${intentLabel(property)}</span><h3 id="propertyModalTitle">${property.title}</h3><p class="modal-location"><span class="location-label">Location</span><span>${property.location} · ${property.area}</span></p><p class="modal-price-line">${formatPrice(property.price,property.priceType)}</p><section class="modal-specs-grid"><article class="spec-card"><h5>Category</h5><p>${getSpecValue(property, 'category')}</p></article><article class="spec-card"><h5>Beds</h5><p>${getSpecValue(property, 'beds')}</p></article><article class="spec-card"><h5>Baths</h5><p>${getSpecValue(property, 'baths')}</p></article><article class="spec-card"><h5>Area</h5><p>${getSpecValue(property, 'size')}</p></article></section><section class="modal-copy-section"><h4>Overview</h4><p class="modal-description">${polishDescription(property)}</p></section><section class="modal-copy-section"><h4>Key Features</h4><ul class="feature-list">${property.features.map((feature) => `<li>${feature}</li>`).join('')}</ul></section><article class="agent-card"><h4>Your Eastleigh Agent</h4><p><strong>Nadia Yusuf</strong><span>Senior Property Advisor</span></p><p>Call: 0700 370 377</p></article><div class="modal-actions"><button type="button" class="btn ${isSaved ? '' : 'btn-gold'} save-modal">${isSaved ? 'Saved Property ♥' : 'Save Property'}</button><button type="button" class="btn btn-gold contact-agent">Contact Agent</button><a class="btn btn-gold" target="_blank" rel="noopener" href="https://wa.me/254700370377?text=${message}">WhatsApp Agent</a><button type="button" class="btn btn-gold schedule-viewing">Schedule Viewing</button></div><div class="contact-toast" hidden>Agent Nadia Yusuf · 0700 370 377 · eastleigh.demo@broker.co.ke</div>${formPanel}${successPanel}</div></div></div></section>`;
  document.body.appendChild(modal);
}
...
document.addEventListener('click',(event)=>{
 ...
 if(event.target.id==='myListingsBtn'){ closeAccountPanel(); openMyListings(); }
 ...
 if(event.target.closest('#accountPanelModal .modal-close')||event.target.id==='accountPanelModal') closeAccountPanel();
...
try {state.saved = new Set(JSON.parse(localStorage.getItem(STORAGE_KEYS.saved) || '[]'));state.recent = JSON.parse(localStorage.getItem(STORAGE_KEYS.recent) || '[]').filter(id=>getProperty(id)); state.user=getStoredSession(); if(!state.user) localStorage.removeItem(STORAGE_KEYS.session); const draft=JSON.parse(localStorage.getItem(STORAGE_KEYS.draft)||'{}'); state.listingDraft=draft||{}; state.draftImages=draft.images||[]; const userListings=JSON.parse(localStorage.getItem(STORAGE_KEYS.listings)||'[]'); userListings.forEach((item,idx)=>properties.push(userListingToProperty(item,idx)));} catch {}
